...
LOCKFILE="/tmp/matrix_sleep_monitor.lock"
if [ -e "$LOCKFILE" ]; then
	PID=$(cat "$LOCKFILE")
...
CONFIG_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/matrix/config/system.json"
...
get_cmd() {
	...
	if [ -f "$CONFIG_FILE" ]; then
		if [ "$type" == "before" ]; then
			...
			jq -r '.idle.general.after_sleep_cmd // "matrix screen on"' "$CONFIG_FILE"
		fi
	else
		if [ "$type" == "before" ]; then
			echo "loginctl lock-session"
		else
			echo "matrix screen on"
		fi
	fi
}
