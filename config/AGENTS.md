...
## OVERVIEW
Reactive, file-backed configuration system built on `Quickshell.Io`. Source of truth for all shell modules. Stores JSON in `~/.config/matrix/config/`. Gracefully handles missing/malformed files by falling back to hardcoded defaults.
